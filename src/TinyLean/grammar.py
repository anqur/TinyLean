...
expr, function_type, function, call, paren_expr = map(
    lambda n: Forward().set_name(n),
    "expr function_type function call paren_expr".split(),
)

type_ = Group(TYPE).set_name("type")
type_.set_parse_action(lambda l, r: ast.Type(l))

placeholder = Group(UNDER).set_name("placeholder")
placeholder.set_parse_action(lambda l, r: ast.Placeholder(l, True))

ref = Group(IDENT).set_name("ref")
ref.set_parse_action(lambda l, r: ast.Ref(l, r[0][0]))

expr <<= function_type | function | call | paren_expr | type_ | placeholder | ref
